...
describe("CalculatorPage Integration", () => {
  ...
  it("shows the result panel for a single successful farm", () => {
  ...
  it("updates the Farm ID text input when the user types", async () => {
    const user = UserEvent.setup();
    vi.mocked(useCalculator).mockReturnValue(idleHook);
  ...
  it("updates the spacing X input when the user changes the value", async () => {
    const user = UserEvent.setup();
    vi.mocked(useCalculator).mockReturnValue(idleHook);
